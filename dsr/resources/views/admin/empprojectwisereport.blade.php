...
<div class="row">
                ...
                {!! Form::open(['method' => 'post', 'action' => 'ProjectsController@empprojectwisereport', 'id'=>'form_1', 'files' => true ]) !!} 
                {{ csrf_field() }}
...
<div class="col-lg-12">
					 ...
					 @foreach($finalData as $proj => $final)
						<?php 
						 if($final['TotalEmpOnProject']==0){ 
							 continue;
						 }
						?>
						
	<div class="col-lg-12">
        	<h3 class="page-header1"><a href="javascript:void(0);" onclick="myToggleFunction('dataTables-example{{$proj}}')">{{$allProjects[$proj]}} <span style="color:red;">({{$final['TotalEmpOnProject']}})</span></a></h3>
    </div>
    
					<table width="100%" class="table table-striped table-bordered table-hover" style="display:none;visiblity:hidden;" id="dataTables-example{{$proj}}" >
                        <thead>
                            <tr>
                                <th>Eid</th>
                                <th>Name</th>
                        ...
                        <tbody>
                        @foreach($final['employees'] as $empolyee)
                            <tr class="odd gradeX">
                                ...
                                <?php if($user->designation=='Administrator' || $user->designation=='MD' || $user->designation=='Accountant' || $user->designation=='Sr Programmer' ){ ?>
                                ...
                                <td>
                                <?php if(isset($final['managerName'][$empolyee->emp_id])){ ?>
                                        {{$final['managerName'][$empolyee->emp_id]}}
                                <?php } ?>
                                </td>
...
-->
function myToggleFunction(elId) {
	$("#"+elId).toggle();
} 
